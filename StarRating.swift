...
public final class StarRating: NSObject, UITextViewDelegate, MFMailComposeViewControllerDelegate {
    ...
    private var bgMessageView = UIView()
    ...
    private var submitConstraintY: NSLayoutConstraint?
    private var submitKeyBoardY: NSLayoutConstraint?
    private var isSetCenterY = false
    private var frameMinYsubmit: CGFloat = 0
    private override init(){
        
    }
    
    func setupKeyBoard(vc: UIViewController) {
    ...
    @objc func keyboardWillShow(_ notification: Notification) {
        if let keyboardFrame = notification.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? CGRect,
           let duration = notification.userInfo?[UIResponder.keyboardAnimationDurationUserInfoKey] as? TimeInterval {
            ...
            guard let submitView = submitView
            else {return}
            if !isSetCenterY {
                frameMinYsubmit = submitView.frame.minY
            }
            let heightMinYsubmit = keyboardHeight - frameMinYsubmit
            UIView.animate(withDuration: duration) {
                if heightMinYsubmit > 0 {
                    self.submitKeyBoardY?.isActive = false
                    self.isSetCenterY = true
                    self.submitConstraintY?.isActive = false
                    self.submitKeyBoardY = submitView.centerYAnchor.constraint(equalTo: self.darkBackground.centerYAnchor, constant: -heightMinYsubmit)
                    self.submitKeyBoardY?.isActive = true
                }
            }
            print("submitView.frame.minY\(frameMinYsubmit)")
            print("heightMinYsubmit count\(heightMinYsubmit)")
            print("keyboard count\(keyboardFrame.height)")
        }
    }
    ...
    @objc func keyboardWillHide(_ notification: Notification) {
        if let duration = notification.userInfo?[UIResponder.keyboardAnimationDurationUserInfoKey] as? TimeInterval {
            UIView.animate(withDuration: duration) {
                if self.isSetCenterY {
                    self.submitKeyBoardY?.isActive = false
                    self.submitConstraintY?.isActive = true
                }
            }
        }
        print("keyboard hinde")
    }
...
//        alertView.heightAnchor.constraint(equalToConstant: 288).isActive = true
        ...
        NSLayoutConstraint.activate([
            ...
            self.content.heightAnchor.constraint(greaterThanOrEqualToConstant: 50)
        ])
        ...
        NSLayoutConstraint.activate([
            self.remind.topAnchor.constraint(equalTo: self.submit.bottomAnchor, constant: 10),
            ...
            self.remind.heightAnchor.constraint(greaterThanOrEqualToConstant: 48)
        ])
    ...
    @objc private func submitAction() {
        if darkBackground != nil {
            if alertView != nil && star <= 3 {
                ...
                submitConstraintY = submitView.centerYAnchor.constraint(equalTo: darkBackground.centerYAnchor)
                submitConstraintY?.isActive = true
                ...
                submitView.addSubview(bgMessageView)
                bgMessageView.layer.backgroundColor = UIColor(red: 0.961, green: 0.961, blue: 0.961, alpha: 1).cgColor
                bgMessageView.layer.cornerRadius = 10
                bgMessageView.translatesAutoresizingMaskIntoConstraints = false
                NSLayoutConstraint.activate([
                    bgMessageView.topAnchor.constraint(equalTo: nameAppLabel.bottomAnchor, constant: 20),
                    bgMessageView.leadingAnchor.constraint(equalTo: submitView.leadingAnchor, constant: 16),
                    bgMessageView.trailingAnchor.constraint(equalTo: submitView.trailingAnchor, constant: -16),
                    bgMessageView.heightAnchor.constraint(equalToConstant: 100)
                ])
                
                messageView = UITextView()
                guard let messageView = self.messageView else { return }
                messageView.layer.backgroundColor = UIColor.clear.cgColor
                ...
                NSLayoutConstraint.activate([
                    messageView.topAnchor.constraint(equalTo: bgMessageView.topAnchor, constant: 0),
                    messageView.leadingAnchor.constraint(equalTo: bgMessageView.leadingAnchor, constant: 0),
                    messageView.trailingAnchor.constraint(equalTo: bgMessageView.trailingAnchor, constant: 0),
                ...
                NSLayoutConstraint.activate([
                    countText.trailingAnchor.constraint(equalTo: bgMessageView.trailingAnchor, constant: -16),
                    countText.topAnchor.constraint(equalTo: messageView.bottomAnchor, constant: 0)
                ])
                ...
                NSLayoutConstraint.activate([
                    cancelButton.topAnchor.constraint(equalTo: bgMessageView.bottomAnchor, constant: 25),
                    cancelButton.leadingAnchor.constraint(equalTo: submitView.leadingAnchor, constant: 16),
                    cancelButton.widthAnchor.constraint(equalToConstant: (submitView.frame.width-30)/2),
                    cancelButton.bottomAnchor.constraint(equalTo: submitView.bottomAnchor, constant: -16),
                    cancelButton.heightAnchor.constraint(greaterThanOrEqualToConstant: 40)
                ])
                ...
                NSLayoutConstraint.activate([
                   sendButton.topAnchor.constraint(equalTo: bgMessageView.bottomAnchor, constant: 25),
                    sendButton.trailingAnchor.constraint(equalTo: submitView.trailingAnchor, constant: -16),
                    sendButton.widthAnchor.constraint(equalToConstant: (submitView.frame.width-30)/2),
                ...
                if let url = URL(string: "itms-apps://itunes.apple.com/app/id\(EventConstraint.APP_ID)?action=write-review"){
...
//        darkBackground.removeFromSuperview()
    ...
    func sendEmail() {
            if MFMailComposeViewController.canSendMail() {
                ...
                mailComposeVC.setToRecipients([emailAddress ?? EventConstraint.MAIL_TO])
